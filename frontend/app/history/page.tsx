...
import { getEpisodeHistory, resumeEpisode, getProcessingQueue, EpisodeListItem, QueuedEpisode } from '@/lib/api'
import { Card, CardContent } from '@/components/ui/card'
import { Button } from '@/components/ui/button'
import { Badge } from '@/components/ui/badge'
import { Plus, Mic, Loader2, RotateCcw, Clock, Podcast, ListOrdered } from 'lucide-react'
import { cn } from '@/lib/utils'
import { dateFormatters } from '@/lib/date'

type StatusFilter = 'all' | 'completed' | 'processing' | 'failed' | 'queue'
...
export default function HistoryPage() {
  ...
  const fetchQueue = async () => {
    ...
    try {
      ...
      setQueuedEpisodes(data.episodes)
  ...
  const filterTabs: { key: StatusFilter; label: string; icon?: React.ReactNode }[] = [
    { key: 'all', label: 'All' },
    { key: 'queue', label: 'Queue', icon: <ListOrdered className="h-4 w-4 mr-1" /> },
    { key: 'completed', label: 'Completed' },
    { key: 'processing', label: 'Processing' },
    { key: 'failed', label: 'Failed' },
  ]

  const displayTotal = filter === 'queue' ? queueTotal : total
    ...
    <div className="max-w-4xl mx-auto">
      <div className="flex items-center justify-between mb-6">
        <div>
          ...
          <p className="text-sm text-muted-foreground mt-1">{displayTotal} episode{displayTotal !== 1 ? 's' : ''}</p>
        </div>
        ...
        queuedEpisodes.length === 0 ? (
          <Card>
            <CardContent className="text-center py-12">
              ...
              <p className="text-muted-foreground mb-4">No episodes in queue</p>
              <p className="text-sm text-muted-foreground">
                Select episodes from your subscriptions to add them to the processing queue.
              </p>
            </CardContent>
          </Card>
        ) : (
                ...
                <CardContent className="p-4">
                  ...
                  <div className="mt-3 flex items-center justify-between">
                    <span className="text-xs text-muted-foreground">
                      Added {dateFormatters.activityTime(episode.created_at)}
                    </span>
